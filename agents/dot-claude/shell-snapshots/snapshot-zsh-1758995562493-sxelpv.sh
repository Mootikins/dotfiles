...
gcobf-list () {
	...
	git branch -a --color=always | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} -v '/HEAD\s' | sort | fzf --ansi --multi --tac --preview-window right:70% --preview 'git log --oneline --graph --date=short --color=always --pretty="format:%C(auto)%cd %h%d %s" $(sed s/^..// <<< {} | cut -d" " -f1) | head -'$LINES | sed 's/^..//' | cut -d' ' -f1 | sed 's#^remotes/##'
}
gcohf-list () {
	is_in_git_repo || return
	git log --date=short --format="%C(green)%C(bold)%cd %C(auto)%h%d %s (%an)" --color=always | fzf --ansi --no-sort --reverse --multi --bind 'ctrl-s:toggle-sort' --header 'Press CTRL-S to toggle sort' --preview 'grep -o "[a-f0-9]\{7,\}" <<< {} | xargs git show --color=always | head -'$LINES | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} -o "[a-f0-9]\{7,\}"
	if [ $? -eq 130 ]
	then
		true
	fi
...
glf () {
	git log --graph --exclude='save/*' --exclude='trash/*' --color=always --format="%C(auto)%h%d %s %C(4)%C(bold)%an, %cr" "$@" | fzf --ansi --no-sort --reverse --preview "echo {} |                 grep -o '[a-f0-9]\{7\}' |                 head -1 |                 xargs -I % sh -c 'git show --color=always %'" --bind "enter:execute:(                 grep -o '[a-f0-9]\{7\}'                 | head -1                 | xargs -I % sh -c 'git show --color=always %                 | less -R'                 ) << 'FZF-EOF' {} FZF-EOF"
	if [ $? -eq 130 ]
	then
		true
	fi
...
nvm_prompt_info () {
	alias | /usr/bin/which --tty-only --read-alias --show-tilde --show-dot nvm &> /dev/null || return
...
if ! command -v rg >/dev/null 2>&1; then
  alias rg='/home/moot/.local/share/claude/versions/1.0.128 --ripgrep'
fi
export PATH=/home/moot/.opencode/bin\:/home/moot/.nvm/versions/node/v24.7.0/bin\:/usr/lib64/ccache\:/home/moot/.cargo/bin\:/usr/local/bin\:/usr/bin\:/home/moot/.local/bin/\:/home/moot/go/bin\:/home/moot/dotfiles/scripts\:/home/moot/.cargo/bin/\:/home/moot/Raven/bitbake-docker\:/home/moot/.screenlayout\:/home/moot/.bun/bin
